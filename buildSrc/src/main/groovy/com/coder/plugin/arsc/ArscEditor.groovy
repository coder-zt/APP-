...
public class ArscEditor extends AssetEditor {
    ...
    def readTable() {
        ...
        def sbTable = new StringBuilder()
        sbTable.append("{")
        def t = [:]
        //1.ResTableHeader    资源表的元数据
        //  type:2          标识符：ResType.RES_TABLE_TYPE
        //  headerSize:12   头文件的大小
        //  size:213024     文件的大小
        //  packageCount：1  package数，通常为1
        t.header = header
        t.packageCount = readInt()
        sbTable.append("\"ResTableHeader\":{")
        //type
        sbTable.append("\"type\":")
        sbTable.append(header.type)
        sbTable.append(",")
        //headerSize
        sbTable.append("\"headerSize\":")
        sbTable.append(header.headerSize)
        sbTable.append(",")
        //size
        sbTable.append("\"size\":")
        sbTable.append(header.size)
        sbTable.append(",")
        //packageCount
        sbTable.append("\"packageCount\":")
        sbTable.append(t.packageCount)
        sbTable.append("}")
        sbTable.append(",")
        //2.ResStringPool
        //  header:
        ...
        // styleOffsets
        // isUtf8
        sbTable.append("\"ResStringPool\":")
        t.stringPool = readStringPool()
        sbTable.append(map2Json(t.stringPool))
        sbTable.append(",")
        //3.ResTablePackage
        //  header:
        //      type
        ...
        // lastPublicKey
        // typeIdOffset
        sbTable.append("\"ResTablePackage\":")
        t.package = readPackage(sbTable)
        //3.资源类型字符串池
        //  header:
        //      type
        //      headerSize
        ...
        // styleOffsets
        // isUtf8
        sbTable.append("\"资源类型字符串池\":")
        t.typeStringPool = readStringPool()
        sbTable.append(map2Json(t.typeStringPool))
        //4.资源项名称型字符串池
        //  header:
        //      type
        //      headerSize
        ...
        // styleOffsets
        // isUtf8
        sbTable.append("\"资源项名称型字符串池\":")
        t.keyStringPool = readStringPool()
        sbTable.append(map2Json(t.keyStringPool))
        // 5.RES_TABLE_TYPE_SPEC_TYPE
        // 6.RES_TABLE_TYPE_TYPE
//        t.typeList = readTypeList()//[specs:specs, lib:lib]
        sbTable.append("}")
        return t
    }
    ...
    def readPackage(StringBuilder sbJson) {
        def header = readChunkHeader()
    ...
    String map2Json(Map map){
        ...
        map.keySet().each {
            sbJson.append("\"$it\":")
            if(map[it] instanceof Map){
                sbJson.append(map2Json(map[it]))
            }else if(map[it] instanceof byte[]){
                sbJson.append("\"${bytes2Json(map[it], true)}\"")
            }else if(map[it] instanceof List){
                sbJson.append("${list2Json(map[it], it=="strings")}")
            }else{
                sbJson.append(map[it])
            }
    ...
    String list2Json(List list, boolean getString){
        if(getString){
        ...
        list.each {
            if(it instanceof Map){
                sbJson.append(map2Json(it))
            }else if(it instanceof List){
                sbJson.append(list2Json(it, true))
            }else if(it instanceof byte[]){
                sbJson.append(bytes2Json(it, false))
            }else{
                sbJson.append(it)
            }
    ...
    String bytes2Json(byte[] list, boolean getString){
        if(getString){
            return getUtf8String(list)
        }
        StringBuilder sbJson = new StringBuilder()
        ...
        while (offset < length) {
            ...
            switch (tt.header.type) {
                case ResType.RES_TABLE_TYPE_SPEC_TYPE:
                    println("ResType.RES_TABLE_TYPE_SPEC_TYPE")
                    tt.id = readByte()
                    tt.res0 = readByte()
                    tt.res1 = readShort()
                ...
                case ResType.RES_TABLE_TYPE_TYPE:
                    println("ResType.RES_TABLE_TYPE_TYPE")
                    tt.id = readByte()
                    tt.res0 = readByte()
                    tt.res1 = readShort()
                    tt.entryCount = readInt()
                ...
                case ResType.RES_TABLE_LIBRARY_TYPE:
                    println("ResType.RES_TABLE_LIBRARY_TYPE")
                    tt.count = readInt()
                    tt.entries = []
